...
//    private FaceDet mFaceDet = null;
    ...
    private static FaceSwap faceSwap = null;
    private Mat hairModelImageMat;
    private Mat selfieImageMat;
    private Context context;

    private FaceSwap(Context context) {
        this.context = context;
    }

    public static FaceSwap getInstance(Context context){
        if(null == faceSwap){
            faceSwap = new FaceSwap(context);
        }

        return faceSwap;
    }

    public Bitmap swap(Bitmap selfieImage, Bitmap hairModelImage) {
        Log.i(TAG, "swap called");

        hairModelImageMat = new Mat(hairModelImage.getWidth(), hairModelImage.getHeight(), CV_8UC3);
        selfieImageMat = new Mat(selfieImage.getWidth(), selfieImage.getHeight(), CV_8UC3);
        ...
        Mat warpedSelfieImageMat = warp(selfieImageMat, procrustes, hairModelImageMat);
...
//        Mat combinedFaceMask = getFaceMask(warpedSelfieImageMat, sourcefaceLandmarksAug);
        Mat combinedFaceMask = Mat.ones(warpedSelfieImageMat.size(),
        ...
        Bitmap newLook = Bitmap.createBitmap(newLookMat.width(),
                newLookMat.height(), Bitmap.Config.ARGB_8888);

        Utils.matToBitmap(newLookMat, newLook);
...
//                new Scalar(182, 130, 29), 4);
    ...
    public Mat transformationFromPoints(ArrayList<Point> selfieImageLandmarks, ArrayList<Point> hairModelImageLandmark){



        //Todo 2: get help here
        /**
         @staticmethod
        ...
        double [][] points1 = new double[68][2];
        double [][] points2 = new double[68][2];
        for (int i = 0; i < 68; i++){

            point = selfieImageLandmarks.get(i);

            points1[i][0] = point.x;
            points1[i][1] = point.y;

            point = hairModelImageLandmark.get(i);
            points2[i][0] = point.x;
            points2[i][1] = point.y;
        }
    ...
    /**
     ...
     return np.vstack([np.hstack(((s2 / s1) * r, c2.T - (s2 / s1) * r * c1.T)), np.matrix([0., 0., 1.])])
      * @param selfieImageMat
     * @param hairModelImageMat
     */

    public Mat warp(Mat selfieImageMat, Mat M, Mat hairModelImageMat){
        Mat tempImage = new Mat(hairModelImageMat.size(), hairModelImageMat.type());

//        warpAffine(selfieImageMat, tempImage, M, selfieImageMat.size());

        warpAffine(selfieImageMat, tempImage, M,
                new Size(selfieImageMat.width(), selfieImageMat.height()),
                WARP_INVERSE_MAP, BORDER_TRANSPARENT, new Scalar(1, 1, 1));
...
/*
        List<MatOfInt> faceConvexHull = new ArrayList<>(points.size());

        for(int i = 0; i < points.size(); i++){

            faceConvexHull.add(i, new MatOfInt());
            convexHull(points.get(i), faceConvexHull.get(i));
        }*/

        fillPoly(faceMask, points, new Scalar(225, 225, 225));
//        Core.transpose(faceMask, faceMask);
//
        return faceMask;
    }
    /**
    ...
    public Mat edgeRefinement(Mat sourceImage, Mat destinationImage, Mat sourcefaceMask,
        ...
        for(Point point : hairfaceLandMarks){
            points.add(new MatOfPoint(new org.opencv.core.Point(point.x, point.y)));
        }
...
//                combinedFaceMask, sourcefaceLandmarksAug);
//
        Mat edgeBlended = new Mat(destinationImage.rows(), destinationImage.cols(), CvType.CV_8UC4);
//        Photo.seamlessClone(sourceImage, destinationImage, sourcefaceMask,
//                center, edgeBlended, NORMAL_CLONE);

        edgeBlended = destinationImage;
        /**
